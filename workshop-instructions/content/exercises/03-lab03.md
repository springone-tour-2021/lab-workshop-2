...
The `Ctrl-U` is used in the Filetree view to show/hide unmodified files.
...
For the latter block, notice also the configuration that you can use to influence how the image is created.
```execute-1
...
```execute-2
pack build configserver:pk -p configserver -e BP_JVM_VERSION=16.* -e BP_OCI_TITLE="Discovery Server"
pack build eurekaserver:pk -p eurekaserver -e BP_JVM_VERSION=16.* -e BP_OCI_TITLE="Discovery Server"
```
