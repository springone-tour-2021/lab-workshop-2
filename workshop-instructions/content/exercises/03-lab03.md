...
In the second stage, it uses a different base image ith only the JRE in it, since the JDK and maven are not needed at runtime, and it copies the executable jar to it. 
...
Omissions also represent Dockerfile author decisions (e.g. no .dockerfile, no LABELs, etc).
...
Cloud Native Buildpacks (_CNB_, or _buildpacks_, for short) is a CNCF project that as establishe jointly by Pivotal and Heroku in 2018. 
...
The default buildpack is the `paketobuildpacks/builder:base`. 
Paketo Buildpacks are an evolution of the Cloud Founry builpacks.
```execute-1
...
You can see each layer of the base runtime image (from the second build stage).

You can use the `pack image inspect` command to get some more information about the second image.
```execute-1
pack inspect blueorgreenservice:pk
```

You can see precise information about the base image an buildpacks used for the build (akin to the first build stage in our Dockerfile), as well as the base image and start command in the runtime image (akin to the second build stage in our Dockerfile).
...
pack build eureka-server:pk -p eureka-server
```
...
```
pack build eureka-server:pk -p eureka-server -e BP_JVM_VERSION=16.* -e BP_OCI_TITLE="Discovery Server"
```

Now use `dive` to explore the layers of the eureka-server image. 
...
```execute-1
docker tag eureka-server:pk $REGISTRY_HOST/color-app/eureka-server
docker push $REGISTRY_HOST/color-app/eureka-server
...
```execute-1
curl -i -k -X GET "https://$REGISTRY_HOST/api/search?q=color-app/
```
