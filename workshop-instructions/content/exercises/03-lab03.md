...
You can skip Config Server and Eureka Server, as you won't be needing these on Kubernetes.
...
Next, create a configuration file to enable Spring Cloud Kubernetes for a `kubernetes` profile.
Since you will not be using Eureka Server on Kubernetes, you should also disable Eureka client behavior.
```execute-1
yq e -n '.spring.cloud.kubernetes.enabled = true' > ~/color-app/config-files/application-kubernetes.yml
yq e '.eureka.client.enabled = false' -i ~/color-app/config-files/application-kubernetes.yml

cat ~/color-app/config-files/application-kubernetes.yml
```

Conversely, in the default application.yml, you can disable Spring Cloud Kubernetes.
This will preserve the expected behavior for non-Kubernetes deployments.
```execute-1
yq e '.spring.cloud.kubernetes.enabled = false' -i ~/color-app/config-files/application.yml

cat ~/color-app/config-files/application.yml
```
