...
We need a way to access the authgateway app from a browser, so we must
expose the service via an ingress.  Create a new ingress using the following 
action.

```editor:append-lines-to-file
file: ~/color-app/k8s/lift-and-shift/authgateway-ingress.yaml
text: |
...
kubectl apply -f k8s/lift-and-shift/authgateway-ingress.yaml
```
