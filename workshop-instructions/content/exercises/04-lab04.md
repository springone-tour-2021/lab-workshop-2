...
kubectl create deployment eureka \
            --image $REGISTRY_HOST/color-app/eureka \
            -o yaml --dry-run=client > k8s/lift-and-shift/eureka-deployment.yaml
...
```execute-1
kubectl create service clusterip eureka \
            --tcp 80:8080 \
            -o yaml --dry-run=client > k8s/lift-and-shift/eureka-service.yaml
