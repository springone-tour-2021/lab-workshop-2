...
after: 11
```
Notice that `server.port: 8080`. 
On Kubernetes, you don't need to ensure that each app runs on a unique port.
This means you don't need the app-specific YAML configuration files for Auth Gateway, Routing Gateway, and Frontend, since this is the only property that was being set in those files. 
For those apps, you can simply provide the common config file to the respective containers.

Start by creating a ConfigMap from the common `application.yml` file.
```execute-1
kubectl create configmap application-config \
        --from-file=config-files/application.yml \
        -o yaml --dry-run=client > k8s/lift-and-shift/application-configmap.yaml
```

Review the resulting file.
```execute-1
cat k8s/lift-and-shift/application-configmap.yaml
```
...
Do the same for the Routing Gateay and Frontend.
```editor:insert-value-into-yaml
...
In the case of the backend service, you need to retain the configuration file with profile-specific settings.
Take a moment to review the original file.
```editor:open-file
...
kubectl create configmap blueorgreen-config \
        ...
        -o yaml --dry-run=client > k8s/lift-and-shift/blueorgreen-configmap.yaml
```

Review the resulting file.
```execute-1
cat k8s/lift-and-shift/blueorgreen-configmap.yaml
```
...
> Note: Notice that we are adding `blueorgreen` as an additional profile. This will enable Spring Boot to automatically use a properties file called application-blueorgreen.yml. We will use this naming convention when we mount `config-files/blueorgreen.yml`.

Add the following templated environment variable to the manifest to set the active profiles.
You also need to set SERVER_PORT=8080 here for this app, since the value in `application.yml` will be overwritten by the value in `application-blueorgreen.yml`.
```editor:insert-value-into-yaml
file: ~/color-app/k8s/lift-and-shift/blueorgreen-deployment.template
path: spec.template.spec.containers[0]
value:
...
services.  This will also need to be templated depending on the `blueorgreen` service we are deploying.
...
```editor:open-file
file: ~/color-app/k8s/lift-and-shift/blue-deployment.yml
```
...
```execute-1
kubectl apply -f k8s/lift-and-shift/application-configmap.yaml
kubectl apply -f k8s/lift-and-shift/blueorgreen-configmap.yaml
