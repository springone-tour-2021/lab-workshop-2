...
## Configuration: `blueorgreenservice`

The `blueorgreenservice` will use the same ConfigMap as the other apps, but it also has additional configuration specified in the application-specific file.
...
Review the resulting file.
```execute-1
cat k8s/blueorgreen-configmap.yaml | yq eval
```
