...
nohup ./mvnw spring-boot:run -Dspring.profiles.active=green > green-logs.txt 2>&1 </dev/null &
GREEN_PID="$!"

nohup ./mvnw spring-boot:run -Dspring.profiles.active=slowgreen > slowgreen-logs.txt 2>&1 </dev/null &
SLOWGREEN_PID="$!"

nohup ./mvnw spring-boot:run -Dspring.profiles.active=yellow > yellow-logs.txt 2>&1 </dev/null &
YELLOW_PID="$!"
...
They all use the same application name, `BLUEORGREEN`, and each adds an endpoint to the `Status` column with a unique port number.
```dashboard:open-dashboard
name: Eureka
```
...
After logging in, refresh the page several times by right clicking with your mouse and selecting `Reload Frame` to refresh as in the following picture.
![alt_text](images/refresh-dash.png "Right click and click Reload Frame to refresh")

You should see the color rotate between blue, green, and the slow green, which takes 5 seconds to load.


- Right click and select 'Back' to return to the logg in page.
