...
```execute-1
cd configserver && nohup ./mvnw spring-boot:run > log.out 2>&1 </dev/null &
cd eurekaserver && nohup ./mvnw spring-boot:run > log.out 2>&1 </dev/null &
```
...
```execute-1
cd blueorgreenservice && nohup ./mvnw spring-boot:run -Dspring-boot.run.profiles=blue > log-blue.out 2>&1 </dev/null &
```
...
```execute-1
cd blueorgreenservice && nohup ./mvnw spring-boot:run -Dspring-boot.run.profiles=green > log-green.out 2>&1 </dev/null &
cd blueorgreenservice && nohup ./mvnw spring-boot:run -Dspring-boot.run.profiles=slowgreen > log-slowgreen.out 2>&1 </dev/null &
cd blueorgreenservice && nohup ./mvnw spring-boot:run -Dspring-boot.run.profiles=yellow > log-yellow.out 2>&1 </dev/null &
```
...
```execute-1
cd blueorgreenfrontend && nohup ./mvnw spring-boot:run -Dspring-boot.run.profiles=local > log.out 2>&1 </dev/null &
cd blueorgreengateway && nohup ./mvnw spring-boot:run > log.out 2>&1 </dev/null &
cd authgateway && nohup ./mvnw spring-boot:run > log.out 2>&1 </dev/null &
```
...
Notice that no green never takes 5 seconds to appear. Instead each time `slowgreen` is called, the color `red` appears after only one second instead.
...
For example, for the `blue` app, `spring.application.name=blueorgreen` and `profiles=blue`, so the configuration is created using an overlay of the following files: `config-files/application.yml`, `config-files/blueorgreenn.yml`, and `config-files/blueorgreenn-blue.yml`.
...
The routing gateway will wait about one second for a color service instance to return. If it does not receive a response, it time out and resorts to a fallback action, which returns a hard-coded value of "red".
...
Scroll dowwn the page to see the `colorfallback` method, wwhich returns a hard-coded value of `red`.
