...
nohup ./mvnw spring-boot:run 2>&1 > blue-logs.txt & 
BLUE_PID="$!"
cd ..
```
Check status of your `blue` Color Application:
```execute-2
cat  ~/color-app/blueorgreenservice/blue-logs.txt 
```
...
nohup ./mvnw spring-boot:run 2>&1 > green-logs.txt & 
GREEN_PID="$!"

export SPRING_PROFILES_ACTIVE=slowgreen
nohup ./mvnw spring-boot:run 2>&1 > slowgreen-logs.txt & 
SLOWGREEN_PID="$!"

export SPRING_PROFILES_ACTIVE=yellow
nohup ./mvnw spring-boot:run 2>&1 > yellow-logs.txt & 
YELLOW_PID="$!"

cd ..
```

You can exit this command and `nohub` will keep your apps running in the background.
```terminal:interrupt
session: 1
```
...
nohup ./mvnw spring-boot:run 2>&1 > frontend-logs.txt & 
FRONTEND_PID="$!"
cd ..
```

You can exit this command and `nohub` will keep your app running in the background.
```terminal:interrupt
session: 1
```
...
nohup ./mvnw spring-boot:run 2>&1 > gateway-logs.txt &
GW_PID="$!"
cd ..
```
...
```execute-1
cd authgateway
nohup ./mvnw spring-boot:run 2>&1 > authgateway-logs.txt &
AUTHGW_PID="$!"
cd ..
```
...
Check the Eureka dashboard again to validate that all four instances of the `blueorgreen` app are registered.
```dashboard:open-dashboard
name: Eureka
```
