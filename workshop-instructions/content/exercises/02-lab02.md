...
```execute-2
cat  ~/color-app/eureka/eureka-logs.txt
```
...
You can exit this command and `nohup` will keep your apps running in the background.
```terminal:interrupt
session: 1
```
...
You can exit this command and `nohup` will keep your app running in the background.
```terminal:interrupt
session: 1
```
