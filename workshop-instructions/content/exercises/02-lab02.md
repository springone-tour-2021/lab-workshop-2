...
```execute-1
mkdir -p logs
nohup spring cloud configserver eureka > logs/run-spring-cloud.log 2>&1 </dev/null &
```
...
nohup java -Xmx128M -Dspring.profiles.active=blue \
...
```execute-1
cd blueorgreenservice && \
   nohup java -Xmx128M -Dspring.profiles.active=green \
       -jar target/blueorgreen-0.0.1-SNAPSHOT.jar \
       > ../logs/run-green.log 2>&1 </dev/null &

cd blueorgreenservice && \
   nohup java -Xmx128M -Dspring.profiles.active=slowgreen \
       -jar target/blueorgreen-0.0.1-SNAPSHOT.jar \
       > ../logs/run-slowgreen.log 2>&1 </dev/null &

cd blueorgreenservice && \
   nohup java -Xmx128M -Dspring.profiles.active=yellow \
       -jar target/blueorgreen-0.0.1-SNAPSHOT.jar \
...
nohup java -Xmx128M -Dspring.profiles.active=local \
